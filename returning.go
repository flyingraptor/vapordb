...
func execDMLReturning(db *DB, sql string, retCols string, forceWipeOnSchemaConflict bool) ([]Row, error) {
	rewritten, conflictCols, doNothing, upsertWhere := rewriteOnConflict(rewriteAnyAll(rewriteFullOuterJoins(rewriteFilterAggregates(rewriteILIKE(rewriteJSONOps(rewriteDoubleQuotedIdents(sql)))))))
