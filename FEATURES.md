# vapordb — Features & SQL Reference

Complete feature documentation and SQL reference for vapordb. For installation and a quick start see the [README](README.md); for known limitations see [LIMITATIONS.md](LIMITATIONS.md); for release history see [CHANGELOG.md](CHANGELOG.md).
...
## Declaring a target database

Declaring a target is **optional** and fully backward compatible. The default is
`TargetGeneric`, which does no checking at all — identical to vapordb's
historical behaviour. Declare a target only when you already know which real
database you intend to migrate to and want vapordb to keep you honest along the
way.

```go
db := vapordb.New(vapordb.WithTarget(vapordb.TargetPostgres))
// or vapordb.TargetMySQL
```

With a target set, every `Query` / `Exec` (and the named variants) is checked
for SQL that would **not** port cleanly to that database. This is a **warn-only
lint** — it never changes execution or results, and the permissive prototyping
core (schema inference, type widening) is untouched.

### What gets flagged

| Target | Flags constructs such as |
|--------|--------------------------|
| `TargetPostgres` | backtick identifiers, `ON DUPLICATE KEY UPDATE`, `IFNULL()`, `RLIKE` / `REGEXP`, `LIMIT offset, count`, `DATE_FORMAT()` |
| `TargetMySQL` | double-quoted identifiers, `ON CONFLICT`, `ILIKE`, `::` casts, `@>` / `<@`, `= ANY(…)` / `<> ALL(…)`, `NULLS FIRST/LAST`, `RETURNING` |

Keywords that appear inside string literals never trigger a warning.

### Reading warnings

Warnings are always accumulated and retrievable, and can optionally be streamed
live as they occur:

```go
db := vapordb.New(
    vapordb.WithTarget(vapordb.TargetMySQL),
    vapordb.WithPortabilityWarner(func(w vapordb.PortabilityWarning) {
        log.Printf("non-portable: %s — %s", w.SQL, w.Message)
    }),
)

db.Exec(`INSERT INTO t (id) VALUES (1) ON CONFLICT (id) DO NOTHING`)

for _, w := range db.PortabilityWarnings() {
    fmt.Println(w.Message)
}
db.ClearPortabilityWarnings() // discard accumulated warnings
```

Each `PortabilityWarning` carries the offending `SQL`, the `Target` it was
checked against, and a `Message` describing the non-portable construct and the
target's equivalent.

### DDL convenience

With a target declared, `GenerateDDL("")` (empty dialect) uses the target's
dialect automatically:

```go
db := vapordb.New(vapordb.WithTarget(vapordb.TargetPostgres))
ddl, _ := db.GenerateDDL("") // == db.GenerateDDL("postgres")
```

> These portability rules are continuously cross-checked against real
> PostgreSQL and MySQL by the nested `verify` module — see
